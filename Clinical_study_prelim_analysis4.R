...
ggplot(data4[which(data4$Assay=="PGE2"),],aes(time,norm_plac,color=Treatment))+geom_point(size=3, na.rm=TRUE)+geom_line(na.rm=TRUE)+scale_x_log10()+ylim(0,300)+pub_specs +facet_wrap(~SubjectID)+theme(legend.position="bottom")+geom_hline(yintercept=100)+labs(title="COX-2 Activity",x="Hours",y="Plasma PGE2\n (%activity relative to placebo)")
...
ggplot(data4[which(data4$Assay=="PGE2"& data4$Treatment=="Celecoxib"),],aes(time,norm_plac,group=SubjectID,color=SubjectID))+geom_point(size=3, na.rm=TRUE)+geom_line(na.rm=TRUE)+xlim(168,180)+ylim(0,200)+pub_specs +facet_wrap(~SubjectID)+geom_hline(yintercept=100)+theme(legend.position="none")+labs(title="COX-2 Activity",x="Hours",y="COX-2 Activity (%activity relative to placebo)")
